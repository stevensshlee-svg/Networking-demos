...
### Trunk Links
- Ran "int ran e0/0 - 3" followed by "switchport trunk encapsulation dot1q" and "switchport mode trunk" to configure the trunk links encapsulation type to the open source standard Dot1q and to configure each links operational mode as a trunk link. I've repeated this on all switches. This change is needed to allow vtp messages to be sent between switches. 
...
- Ran "spanning-tree vlan 10 root secondary" "spanning-tree vlan 20 root secondary" "spanning-tree vlan 30 root secondary" "spanning-tree vlan 40 root secondary" "spanning-tree vlan 50 root secondary" on SW4 to elect SW4 as the secondary (failover) SW4 for each VLAN
...
3. Validated SW4's convergenve via the output from "do show spanning-tree vlan 10" on SW4
4. Re-enabled links to SW1, causing SW1 to reclaim root bridge after recovergence due to its lower priority
...
- While 50 to 60 seconds may seem minisule, in an enterprise environment that downtime could have significant impact on a business operation. The convergence process of pvst really highlights the direct motivation for Rapid-pvst implementation in enterprise environments.
